...
export default function LoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (err.response?.data?.message) {
                setError(err.response.data.message); // mensaje directo del backend
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (err.response?.data?.message) {
                setRegError(err.response.data.message); // mensaje del backend
            } else if (err.response?.status === 409) {
                setRegError("⚠️ El correo ya está registrado. Intenta con otro.");
            } else if (err.response?.status === 401) {
                setRegError("❌ Datos inválidos. Verifica tu información.");
            } else {
                setRegError("❌ Datos inválidos. Verifica tu información.");
            }
        }
